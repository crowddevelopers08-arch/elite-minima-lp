...
const ITEMS = [
  { icon: ShieldCheck, label: "Advanced Piles Care" },
    ...
    >
      ...
      {reduced ? (
        ...
        <div
          ...
          // Gradient edge fade — items dissolve rather than clip at the sides.
          style={{
        ...
        >
          <motion.div style={{ x: xPct, willChange: "transform" }} className="flex w-max">
            {/* first copy: the one assistive tech reads */}
            <Track />
            {/* second copy: purely visual, keeps the loop seamless */}
            <Track aria-hidden />
          </motion.div>
        </div>
      )}
...
function Track({ "aria-hidden": ariaHidden }: { "aria-hidden"?: boolean }) {
  return (
    <ul aria-hidden={ariaHidden} className="flex shrink-0 items-center">
      {/* repeated so a single copy already overflows the widest viewport */}
      {Array.from({ length: 4 }).flatMap((_, rep) =>
        ITEMS.map(({ icon: Icon, label }) => (
          <li
            key={`${rep}-${label}`}
            className="inline-flex shrink-0 items-center gap-2.5 px-5 sm:px-7"
          >
